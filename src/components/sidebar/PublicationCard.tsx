...
const PublicationCard: React.FC<PublicationCardProps> = ({ 
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full rounded-2xl bg-white shadow-md box-border p-0 grid grid-cols-5 grid-rows-4 gap-0 overflow-hidden cursor-pointer hover:shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 ${
        isHighlighted ? 'ring-4 ring-blue-400 shadow-lg scale-[1.02]' : ''
      }`}
    >
